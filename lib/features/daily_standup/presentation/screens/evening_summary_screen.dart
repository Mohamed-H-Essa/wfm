...
class EveningSummaryScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<EveningSummaryScreen> createState() =>
      _EveningSummaryScreenState();
}
...
class _EveningSummaryScreenState extends ConsumerState<EveningSummaryScreen> {
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      final unplannedTasksData = _unplannedTasks
          .where((task) => task.title != null && task.title!.isNotEmpty)
          .map((task) {
      ...
      final response = await repository.submitEvening(
        ...
        blockerDescription: _blockerDescription?.trim().isEmpty == true
            ? null
            : _blockerDescription?.trim(),
        blockerPriority: _blockerDescription?.trim().isEmpty == true
            ? null
            : _blockerPriority,
      ...
      if (response.success) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content:
                  Text(response.message ?? 'Failed to submit evening summary'),
                  ...
                  child: Form(
                    ...
                    child: Column(
                      ...
                      children: [
                            ...
                            return GestureDetector(
                              ...
                              child: Icon(
                                _productivityRating != null &&
                                        rating <= _productivityRating!
                                    ? Icons.star
                                    : Icons.star_border,
                                size: 40,
                                color: _productivityRating != null &&
                                        rating <= _productivityRating!
                        ...
                        if (_blockerDescription != null &&
                            _blockerDescription!.isNotEmpty) ...[
                          ...
                          Container(
                            ...
                            decoration: BoxDecoration(
                              ...
                              border: Border.all(
                                  color:
                                      IntraZeroColors.warning.withOpacity(0.3)),
                            ),
                            child: Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
