...
class _TimesheetHomeScreenState extends ConsumerState<TimesheetHomeScreen> {
                  ...
                  children: [
                    ...
                    if (!status.isRunning) ...[
                      GradientButton(
                        ...
                        isFullWidth: true,
                        onPressed: () => _showStartTimerOptions(timesheetNotifier),
                      ),
                      ...
                      GradientButton(
                        ...
                        icon: Icons.list_alt,
                        isFullWidth: true,
                        onPressed: () => _handleStartFromPlan(timesheetNotifier),
                      ),
                    ]
  ...
  Future<void> _handleStartFromPlan(TimesheetStatusNotifier notifier) async {
    try {
      ...
      if (!response.success || response.data == null || response.data!.tasks.isEmpty) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('No morning plan tasks available. Please submit your morning plan first.'),
            ...
            child: Column(
              ...
              children: [
                Padding(
                  ...
                  child: Row(
                    children: [
                      Icon(Icons.list_alt, color: IntraZeroColors.primaryGradient.colors.first),
                                ...
                                Expanded(
                                  child: Column(
                                    ...
                                    children: [
                                      ...
                                      Text(
                                        task.projectName ?? task.category ?? 'No project',
                                        style: TextStyle(
                                          fontSize: 13,
                                          color: IntraZeroColors.textSecondary,
                                        ),
      ...
      print('🔄 [TIMESHEET] Starting timer from plan - taskId: ${selectedTask.id}, projectId: ${selectedTask.projectId}');
      final startResponse = await repository.startFromPlan(
        standupTaskId: selectedTask.id,
        projectId: selectedTask.projectId,
      );
      
      print('📥 [TIMESHEET] Start from plan response - success: ${startResponse.success}, message: ${startResponse.message}');
      ...
      if (startResponse.success && startResponse.data != null) {
        ...
        print('📥 [TIMESHEET] Parsed - startedAt: $startedAt, timerId: $timerId, entryId: $entryId');
        ...
        await updatedStatus.when(
          data: (status) async {
            print('📊 [TIMESHEET] Status check - isRunning: ${status.isRunning}, hasEntry: ${status.currentEntry != null}');
            if (status.isRunning && status.currentEntry != null) {
              ...
              print('⚠️ [TIMESHEET] Timer not running, trying fallback to regular start method');
              try {
                ...
                if (mounted) {
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
                      content: Text('Timer started (fallback): ${selectedTask.title}'),
                      backgroundColor: IntraZeroColors.success,
                      duration: const Duration(seconds: 3),
                    ),
                  );
                ...
                if (mounted) {
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
                      content: Text('Failed to start timer. Please try again or contact support.'),
                      backgroundColor: IntraZeroColors.danger,
        ...
        if (mounted) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(startResponse.message ?? 'Failed to start timer from plan'),
              backgroundColor: IntraZeroColors.danger,
              duration: const Duration(seconds: 4),
            ),
          );
